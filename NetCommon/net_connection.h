...
namespace olc
{
	namespace net
	{
		...
		{
		public:
			// Handles ownership requirements of connection object
			enum class owner
			{
				server,
				client
			...
			connection(
				owner parent,					// Owner type of this connection
				asio::io_context& asioContext,	// asio context this connection is associated with
				asio::ip::tcp::socket socket,	// socket connection to remote
				tsqueue<owned_message<T>>& qIn	// Client or Server interface message queue
			) : m_asioContext(asioContext), m_socket(std::move(socket)), m_qMessagesOut(qIn)
			{
				m_nOwnerType = parent;
			}
		...
		private:
			...
			void ReadHeader()
			{
				...
				asio::async_read(
					m_socket,																// Socket to read from
					asio::buffer(&m_msgTemporaryIn.header, sizeof(message_header<T>)),		// ASIO buffer to hold the header data
					[this](std::error_code ec, std::size_t length)							// Lambda called when the header has been read
					{
						if (!ec)
						{
